...
function polyEq(poly1, poly2) {
    ...
    let samePoly =
        poly1 &&
        poly2 &&
        poly1.length != undefined &&
        poly1.length === poly2.length &&
        poly1.every(
            (pt, index) =>
                pt[0] === poly2[index][0] && pt[1] === poly2[index][1],
        );
...
class SegmenterRegion {
    ...
    updateDataFromCanvas() {
        ...
        this.polygon = this.polygonPath.segments.map((s) => [
            Math.round(s.point.x),
    ...
    remove() {
        ...
        let lines = this.segmenter.lines.filter(
            (l) => l.region && l.region.id == this.id,
        );
        lines.forEach((l) => l.update(undefined, undefined, null, undefined));

        this.segmenter.regions.splice(
            this.segmenter.regions.findIndex((e) => e.id == this.id),
            1,
        );
    }
...
class SegmenterLine {
    constructor(
        ...
        if (baseline) {
            if (baseline.segments) {
                // already a paperjs.Path
                this.baselinePath = baseline;
            } else {
                this.baseline = baseline.map((pt) => [
                    Math.round(pt[0]),
                    Math.round(pt[1]),
                ]);
        ...
        this.tooltipText = this.type;
        let tooltipTarget =
            this.baseline !== null ? this.baselinePath : this.maskPath;
    ...
    refresh() {
        this.tooltipText = this.type;
        this.hintColor =
            this.segmenter.directionHintColors[this.type || "None"];
        this.showOrdering();
        this.showDirection();
    ...
    getMaskColor() {
        return this.order % 2
            ? this.segmenter.evenMasksColor
            : this.segmenter.oddMasksColor;
    }
    ...
    makeMaskPath() {
        this.maskPath = new Path({
            ...
            selectedColor: "black",
            visible:
                (this.baseline && this.baseline.length == 0) ||
                this.segmenter.showMasks,
            segments: this.mask,
        });
    ...
    updateDataFromCanvas() {
        ...
        if (this.baselinePath) {
            ...
            this.baseline = this.baselinePath.segments.map((s) => [
                Math.round(s.point.x),
                Math.round(s.point.y),
            ]);
        }
        if (this.maskPath) {
            ...
            this.mask = this.maskPath.segments.map((s) => [
                Math.round(s.point.x),
                Math.round(s.point.y),
            ]);
        }
        ...
        if (
            !polyEq(previous.baseline, this.baseline) ||
    ...
    showOrdering() {
        ...
        if (!this.orderDisplay) {
            ...
            text.content = parseInt(this.order) + 1;
            region.position = {
                x: anchor.x + 5 / this.segmenter.getRatio(),
                y: anchor.y,
            };
            if (this.region) region.fillColor = this.region.color;
            else region.fillColor = "transparent";
    ...
    showDirection() {
        ...
        if (this.baselinePath && this.baselinePath.segments.length > 1) {
            ...
            let vector = this.baselinePath.getNormalAt(
                this.baselinePath.getOffsetOf(anchor.point),
            );
            if (vector) {
                vector.length = this.getLineHeight() / 2;
                this.directionHint.sendToBack();
                this.directionHint.segments = [
                    anchor.point.subtract(vector),
    ...
    getLineHeight() {
        if (this.maskPath) {
            ...
            if (this.baselinePath) {
                return Math.round(
                    Math.abs(this.maskPath.area) / this.baselinePath.length,
                );
            } else {
...
export class Segmenter {
    ...
    bindButtons() {
        ...
        this.deletePointBtn.addEventListener(
            "click",
            function () {
            ...
            if (this.selection.segments.length) {
                ...
                for (let i in this.selection.regions) {
                    ...
                    for (let j in region.polygonPath.segments) {
                        this.movePointInView(
                            region.polygonPath.segments[j].point,
                            delta,
                        );
                    }
                }
    ...
    movePointInView(point, delta) {
        ...
        if (point.x > this.img.naturalWidth / this.scale)
            point.x = this.img.naturalWidth / this.scale;
        if (point.y < 0) point.y = 0;
        if (point.y > this.img.naturalHeight / this.scale)
            point.y = this.img.naturalHeight / this.scale;
    }
    ...
    refresh() {
        ...
        if (paper.view) {
            ...
            if (
                paper.view.viewSize[0] != boundsWidth &&
    ...
    loadLine(line, region) {
        let context = {};
        if (
            (line.baseline !== null && line.baseline.length) ||
    ...
    toggleMasks(force) {
        ...
        for (let i in this.lines) {
            ...
            if (poly) {
                ...
                if (poly.visible && this.lines[i].selected)
                    poly.selected = true;
            }
        }
    }
    ...
    applyRegionMode() {
        if (this.mode == "regions") {
            for (let index in this.regions) {
                let region = this.regions[index];
                region.polygonPath.strokeColor = this.shadeColor(
                    region.color,
                    -50,
                );
    ...
    showContextMenu() {
        ...
        if (this.selection.lines.length) {
            ...
            if (
                this.selection.lines.filter((l) => l.baseline !== null).length >
                1
            ) {
            ...
            if (this.regions.length > 0) {
                if (
                    this.selection.lines.filter((l) => l.region == null)
                        .length > 0
                ) {
        ...
        if (this.selection.regions.length) {
            if (this.regionTypes.length)
                this.setTypeBtn.style.display = "block";
            else this.setTypeBtn.style.display = "none";
        }
    ...
    showTypeSelect() {
        if (this.selection.lines.length) {
            ...
            this.lineTypesSelect.style.left =
                this.setTypeBtn.offsetLeft +
                this.setTypeBtn.clientWidth +
                10 +
                "px";
            this.lineTypesSelect.focus();
            // if all type are the same selects it in the type selector
            if (
                this.selection.lines.every(
                    (line, i, arr) => line.type === arr[0].type,
                )
            ) {
                this.lineTypesSelect.value =
                    this.selection.lines[0].type || "None";
            } else {
            ...
            this.regionTypesSelect.style.left =
                this.setTypeBtn.offsetLeft +
                this.setTypeBtn.clientWidth +
                10 +
                "px";
            ...
            // if all type are the same selects it in the type selector
            if (
                this.selection.regions.every(
                    (reg, i, arr) => reg.type === arr[0].type,
                )
            ) {
                this.regionTypesSelect.value =
                    this.selection.regions[0].type || "None";
            } else {
        ...
        function bindKb(ev) {
            if (ev.keyCode == 27) {
                ...
                if (
                    !isNaN(num) &&
                    num <= document.activeElement.childElementCount
                ) {
                    self.setSelectionType(
                        document.activeElement.children[num].value,
                    );
                    unbindKb.bind(this)();
                    ev.stopPropagation();
                }
    ...
    addToSelection(obj) {
        if (obj instanceof SegmenterLine) {
            if (this.selection.lines.findIndex((e) => e.id == obj.id) == -1)
                this.selection.lines.push(obj);
        } else if (obj instanceof SegmenterRegion) {
            if (this.selection.regions.findIndex((e) => e.id == obj.id) == -1)
                this.selection.regions.push(obj);
        } else {
            // must be a segment
            if (
                this.selection.segments.findIndex(
    ...
    removeFromSelection(obj) {
        if (obj instanceof SegmenterLine) {
            this.selection.lines.splice(
                this.selection.lines.findIndex((e) => e.id == obj.id),
                1,
            );
        } else if (obj instanceof SegmenterRegion) {
            this.selection.regions.splice(
                this.selection.regions.findIndex((e) => e.id == obj.id),
                1,
            );
        } else {
            // must be a segment
            let fi = this.selection.segments.findIndex(
                (e) =>
                    e.path &&
                    obj.path &&
                    e.path.id == obj.path.id &&
    ...
    updateSelectionRectangle(clip, event) {
        clip.bounds.width = Math.max(
            1,
            Math.abs(clip.originalPoint.x - event.point.x),
        );
        ...
        clip.bounds.height = Math.max(
            1,
            Math.abs(clip.originalPoint.y - event.point.y),
        );
        ...
        if (["lines", "masks"].includes(this.mode)) {
            this.lines.forEach(
                function (line) {
                    ...
                    let intersections =
                        line.baselinePath.getIntersections(clip);
                    for (var i = 0; i < intersections.length; i++) {
                        ...
                        if (intersections.length) {
                            ...
                            cut.bringToFront();
                            line.baselinePath.segments.forEach(
                                function (segment) {
        ...
        this.regions.forEach(
            function (region) {
                ...
                if (intersections.length == 2) {
                    ...
                    let newPath = region.polygonPath.subtract(path, {
                        insert: true,
                    });
                    ...
                    let compound = region.polygonPath.subtract(path, {
                        insert: true,
                    });
                    region.polygonPath.removeSegments();
                    compound.children[0].segments.forEach((s) =>
                        region.polygonPath.add(s),
                    );
                    for (let i = 1; i < compound.children.length; i++) {
                        let newRegion = this.createRegion(
                            null,
                            compound.children[i].segments,
                            region.type,
                            null,
                            false,
                        );
                    ...
                    for (var i = 0; i < intersections.length; i += 2) {
                        if (i + 1 >= intersections.length) {
                            ...
                            if (
                                ...
                                line.baselinePath.removeSegments(
                                    0,
                                    newSegment.index,
                                );
                            } else if (
                            ...
                            let normal1 = line.baselinePath.getNormalAt(
                                intersections[i].offset,
                            );
                            let normal2 = line.baselinePath.getNormalAt(
                                intersections[i + 1].offset,
                            );
                            let split = line.baselinePath.splitAt(
                                intersections[i + 1],
                            );
                            let trash = line.baselinePath.splitAt(
                                intersections[i],
                            );
                            trash.remove();
                            ...
                            if (line.maskPath !== null) {
                                ...
                                let ng = line.maskPath.divide(clip, {
                                    insert: false,
                                });
                                clip.remove();
                                ...
                                if (ng.children) {
                                    ...
                                    for (let n in ng.children.slice(0, 3)) {
                                        ...
                                        let det1 = this.getDeterminant(
                                            normal1,
                                            { x: ip.x - a.x, y: ip.y - a.y },
                                        );
                                        let det2 = this.getDeterminant(
                                            normal2,
                                            { x: ip.x - b.x, y: ip.y - b.y },
                                        );
                                        // pattern should be -- / ++ / -+
                                        if (
                                            Math.sign(det1) == Math.sign(det2)
                                        ) {
                                            if (det1 < 0) {
                                                line.maskPath.removeSegments();
                                                ng.children[n].segments.forEach(
                                                    (s) => line.maskPath.add(s),
                                                );
                                                line.maskPath.closePath();
                                            } else {
                                                newMask =
                                                    ng.children[n].segments;
                                            }
                                        }
                                    }
                                }
    ...
    linkSelection() {
        for (let i in this.selection.lines) {
            ...
            for (let j in this.regions) {
                ...
                let center = line.baselinePath.getPointAt(
                    line.baselinePath.length / 2,
                );
    ...
    mergeSelection() {
        if (
            this.selection.lines.filter((sel) => sel.baselinePath === null)
                .length > 0
        ) {
            return;
        }
    ...
    frontendMergeSelection() {
        ...
        while (this.selection.lines.length > 1) {
            let l1 = this.selection.lines[0],
                l2 = this.selection.lines[1];
            ...
            if (l1.maskPath != null && l2.maskPath != null) {
                let closeSeg = l1.maskPath.getNearestLocation(
                    l2.maskPath.interiorPoint,
                );
                l1.maskPath.insertSegments(
                    closeSeg.index + 1,
                    l2.maskPath.segments.slice(0, -1),
                );
            }
    ...
    addToUpdateQueue(items) {
        // {lines:[], regions:[]}
        this.updateQueue.lines = this.updateQueue.lines.concat(
            items.lines || [],
        );
        this.updateQueue.regions = this.updateQueue.regions.concat(
            items.regions || [],
        );
    }
    ...
    setColors() {
        ...
        for (let index in this.lineTypes) {
            ...
            if (this.directionHintColors[type] == null) {
                this.directionHintColors[type] = this.changeHue(
                    choices[1],
                    3 * (index + 1),
                );
            }
        ...
        for (let index in this.regionTypes) {
            ...
            if (this.regionColors[type] == null) {
                this.regionColors[type] = this.changeHue(
                    choices[2],
                    3 * (index + 1),
                );
            }
