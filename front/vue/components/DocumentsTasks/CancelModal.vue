<template>
  <div class="modal fade" :id="id" ref="cancelTasksModal" tabindex="-1" role="dialog" aria-hidden="true">
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          <p>You're about to cancel all pending/running tasks for documents:</p>
            <ul>
              <li v-for="document in documentsTasks" :key="document.pk">
        ...
        <div class="modal-footer">
          <button type="button" class="btn btn-secondary" data-dismiss="modal" :disabled="loading">Abort</button>
          <button type="button" class="btn btn-danger" @click="cancelTasks()" :disabled="loading">{{ loading ? 'Canceling tasks...' : 'Cancel tasks' }}</button>
        </div>
      </div>
    </div>
  </div>
