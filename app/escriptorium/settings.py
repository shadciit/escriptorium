...
ADMINS = [
    (
        os.getenv("DJANGO_SU_NAME", "admin"),
...
TEMPLATES = [
    {
        ...
        ],  # custom homepage dir (volume in docker)
        "APP_DIRS": True,
        "OPTIONS": {
            "context_processors": [
...
STATICFILES_DIRS = [os.path.join(PROJECT_ROOT, "static"), FRONTEND_DIR]
...
LOGGING = {
    ...
    "loggers": {
        ...
        "django": {"handlers": ["file", "console", "mail_admins"]},
        "django.server": {
...
QUOTA_DISK_STORAGE = (
    int(os.environ["QUOTA_DISK_STORAGE"])
    if os.environ.get("QUOTA_DISK_STORAGE")
    else None
)
...
QUOTA_CPU_MINUTES = (
    int(os.environ["QUOTA_CPU_MINUTES"])
    if os.environ.get("QUOTA_CPU_MINUTES")
    else None
)
